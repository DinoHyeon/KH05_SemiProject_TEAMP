...
public class BoardService {
   ...
   public void groupWrite() throws ServletException, IOException {
      ...
      if(idx > 0 && dto.getGroup_idx() != 0) {
         response.sendRedirect("detail?idx=" + dto.getBbs_idx()+"&pageNo=" +  pageNo);
      }
   }
   ...
   public void comunityBbsDelete() throws IOException {
      ...
      if(dao.del(idx) > 0) {
         //관리자가 아니면(일반유저) 삭제시 comunityList 호출
            response.sendRedirect("comunityList?pageNo="+pageNo);
      }  
   }
      ...
      //파라메터 추출
         ...
         if(dao.del(idx) > 0) {
            ...
            if(!request.getSession().getAttribute("loginId").equals("admin")) {
               response.sendRedirect("groupList?pageNo=" + pageNo);
            }else {//관리자면 관리자 그룹게시판 관리페이지 호출
               response.sendRedirect("adminGroupBbsList?pageNo=" + pageNo);
            }
         }  
   }
   ...
   public void list() throws IOException, ServletException {
      ...
      ArrayList<BoardDTO> list = dao.list(keyField, keyWord,(pageNo-1)*10+1, pageNo*10);
   ...
   public void adminList() throws ServletException, IOException {
      ...
      if(request.getParameter("pageNo")!=null) {
       ...
       PageDTO paging = new PageDTO();
        paging.setPageNo(pageNo);
        paging.setPageSize(10);
        paging.setTotalCount(dao.getAdminTotalCount(keyField, keyWord));

      ArrayList<BoardDTO> list = dao.adminList(keyField, keyWord,(pageNo-1)*10+1, pageNo*10);
      System.out.println(list.size());
      request.setAttribute("list",list);
      request.setAttribute("paging",paging);
      request.getSession().setAttribute("pageNo",pageNo);
   ...
   public void groupList() throws ServletException, IOException {
      ...
      if(request.getParameter("pageNo")!=null) {
       ...
       PageDTO paging = new PageDTO();
        paging.setPageNo(pageNo);
        paging.setPageSize(10);
        paging.setTotalCount(dao.getGroupTotalCount(group_idx, keyField, keyWord));

      ArrayList<BoardDTO> list = dao.groupList(group_idx,keyField, keyWord,(pageNo-1)*10+1, pageNo*10);
      System.out.println(list.size());
      request.setAttribute("list",list);
      request.setAttribute("paging",paging);
   ...
   public void adminGroupBbsList() throws ServletException, IOException {
      ...
      if(request.getParameter("pageNo")!=null) {
       ...
       PageDTO paging = new PageDTO();
        paging.setPageNo(pageNo);
        paging.setPageSize(10);
        paging.setTotalCount(dao.getAdminGroupTotalCount(keyField, keyWord));

      ArrayList<BoardDTO> list = dao.adminGroupBbsList(keyField, keyWord,(pageNo-1)*10+1, pageNo*10);
      System.out.println(list.size());
      request.setAttribute("list",list);
      request.setAttribute("paging",paging);
   ...
   public void fileList() throws ServletException, IOException {
    ...
    //게시판별 세션을 이용한 pageNo 구분을 위해 타게시판으로 이동시 세션을지워줌
     ...
     PageDTO paging = new PageDTO();
      paging.setPageNo(pageNo);
      paging.setPageSize(10);
      paging.setTotalCount(dao.getFileTotalCount(group_idx, keyField, keyWord));

     ArrayList<BoardDTO> list = dao.fileList(group_idx,keyField, keyWord,(pageNo-1)*10+1, pageNo*10);
     System.out.println(list.size());
     request.setAttribute("list",list);
     request.setAttribute("paging",paging);
     request.getSession().setAttribute("pageNo",pageNo);
   ...
   public void fileWrite() throws IOException {
      ...
      int pageNo = (int) request.getSession().getAttribute("pageNo");
      System.out.println("파일게시판 페이지세션 = " + pageNo);
      ...
      if(pk > 0) {
         ...
         page = "fileDetail?idx=" + pk + "&pageNo=" + pageNo;
      }
   ...
   public void fileDelete() throws IOException {
      ...
      int pageNo = (int)request.getSession().getAttribute("pageNo");
      if(request.getSession().getAttribute("loginId").equals("admin")) {
         response.sendRedirect("adminFileBbsList?pageNo=" + pageNo);
      }else {
         response.sendRedirect("fileList?pageNo=" + pageNo );
      }
   ...
   public void adminFileBbsList() throws ServletException, IOException {
      ...
      PageDTO paging = new PageDTO();
       paging.setPageNo(pageNo);
       paging.setPageSize(10);
       paging.setTotalCount(dao.getAdminFileTotalCount(keyField, keyWord));

        ArrayList<BoardDTO> list = dao.adminFileBbsList(keyField, keyWord,(pageNo-1)*10+1, pageNo*10);
        System.out.println(list.size());
        request.setAttribute("list",list);
        request.setAttribute("paging",paging);
        request.getSession().setAttribute("pageNo",pageNo);
